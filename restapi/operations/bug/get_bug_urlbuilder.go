...
func (o *GetBugURL) Build() (*url.URL, error) {
	...
	var _path = "/catalog/bugs/{id}"

	id := swag.FormatInt64(o.ID)
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on GetBugURL")
	}
...
func (o *GetBugURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetBugURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetBugURL")
	}
